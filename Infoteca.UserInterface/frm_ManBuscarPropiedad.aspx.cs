...
{
    public partial class frm_ManBuscarPropiedad : Page
    {
        ...
        protected void BorrarPropiedad(object sender, EventArgs e)
        {
            var idTiposFuente = Session["idPropiedad"] != null ? Session["idPropiedad"].ToString(): "0";

            if (!idTiposFuente.Equals("0"))
            {
                var mensajeError = new MensajeError();

                if(PropiedadBL.BorrarPropiedad(int.Parse(idTiposFuente), ref mensajeError)) 
                {
                    controlMensajes.MostrarMensaje(false, "Propiedad eliminada");
                } 
            }
        ...
        {
            var idPropiedad = gvPerson.Rows[e.NewEditIndex].Cells[0].Text;

            if (idPropiedad != null)
            {
                Response.Redirect("/frm_ManEditarPropiedad.aspx?idPropiedad=" + idPropiedad, false);
            }
        }

        protected void gvPerson_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            var idPropiedad = gvPerson.Rows[e.RowIndex].Cells[0].Text;
            
            Session.Add("idPropiedad", idPropiedad);

            var mensajeError = new MensajeError();

            var propiedad = PropiedadBL.BuscarPropiedad(int.Parse(idPropiedad), ref mensajeError);
            var apariciones = PropiedadBL.BuscarCantidadApariciones(int.Parse(idPropiedad), ref mensajeError);

            borrarMensaje.Text = $"Desea borrar la propiedad: {propiedad.LstrLugar}? Registrada en {apariciones} Noticia(s).";
        ...
        private void BindGridView()
        {
            ...
            var listaTiposFuente = PropiedadBL.BuscarTodosPropiedad(ref mensajeError);
