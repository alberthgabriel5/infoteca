...
{
    public partial class frm_ConBusquedaPorCriterio : Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (Session["CriterioBusqueda"] != null)
                {
                    Criterios.Text = Session["CriterioBusqueda"].ToString();

                    Session["CriterioBusqueda"] = null;
        ...
        protected void BuscarPorCriterio(object sender, EventArgs e)
        {
            Mensaje.Visible = false;

            var mensajeError = new MensajeError();

            if (!string.IsNullOrEmpty(Criterios.Text))
            {
                BindGridView();
            }
            else
            {
                Mensaje.Text = "Por Favor, Ingrese un criterio de busqueda valido";
        ...
        protected void gvNoticia_RowEditing(object sender, GridViewEditEventArgs e)
        {
            ...
            if (idNoticia != null)
            {
                Session.Add("CriterioBusqueda", Criterios.Text);
                Response.Redirect($"/frm_ManVerNoticia.aspx?idNoticia={idNoticia}&PaginaActual=Criterio", false);
            }
        }
        ...
        private void BindGridView()
        {
            ...
            var noticias = NoticiaBL.BuscarPorPalabraClave(Criterios.Text, usuario.GetUserName(), ref mensajeError);
