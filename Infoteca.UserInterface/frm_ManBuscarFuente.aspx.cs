...
{
    public partial class frm_ManBuscarFuente : Page
    {
        ...
        protected void BorrarFuente(object sender, EventArgs e)
        {
            var idTiposFuente = Session["idFuente"] != null ? Session["idFuente"].ToString(): "0";

            if (!idTiposFuente.Equals("0"))
            {
                var mensajeError = new MensajeError();

                if(FuenteBL.BorrarFuente(int.Parse(idTiposFuente), ref mensajeError)) 
                {
                    controlMensajes.MostrarMensaje(false, "Fuente eliminada");
                } 
            }
        ...
        {
            var idFuente = gvPerson.Rows[e.NewEditIndex].Cells[0].Text;

            if (idFuente != null)
            {
                Response.Redirect("/frm_ManEditarFuente.aspx?idFuente=" + idFuente, false);
            }
        }

        protected void gvPerson_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            var idFuente = gvPerson.Rows[e.RowIndex].Cells[0].Text;
            
            Session.Add("idFuente", idFuente);

            var mensajeError = new MensajeError();

            var fuente = FuenteBL.BuscarFuente(int.Parse(idFuente), ref mensajeError);

            borrarMensaje.Text = $"Desea borrar la fuente: {fuente.LstrNombreFuente}?.";
        ...
        private void BindGridView()
        {
            ...
            var listaTiposFuente = FuenteBL.BuscarTodosFuente(ref mensajeError);
