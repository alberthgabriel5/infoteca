...
{
    public partial class frm_ManBuscarVehiculo : Page
    {
        ...
        protected void BorrarVehiculo(object sender, EventArgs e)
        {
            var idTiposFuente = Session["idVehiculo"] != null ? Session["idVehiculo"].ToString(): "0";

            if (!idTiposFuente.Equals("0"))
            {
                var mensajeError = new MensajeError();

                if(VehiculoBL.BorrarVehiculo(int.Parse(idTiposFuente), ref mensajeError)) 
                {
                    controlMensajes.MostrarMensaje(false, "Vehiculo eliminada");
                } 
            }
        ...
        {
            var idVehiculo = gvPerson.Rows[e.NewEditIndex].Cells[0].Text;

            if (idVehiculo != null)
            {
                Response.Redirect("/frm_ManEditarVehiculo.aspx?idVehiculo=" + idVehiculo, false);
            }
        }

        protected void gvPerson_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            var idVehiculo = gvPerson.Rows[e.RowIndex].Cells[0].Text;
            
            Session.Add("idVehiculo", idVehiculo);

            var mensajeError = new MensajeError();

            var vehiculo = VehiculoBL.BuscarVehiculo(int.Parse(idVehiculo), ref mensajeError);
            var apariciones = VehiculoBL.BuscarCantidadApariciones(int.Parse(idVehiculo), ref mensajeError);

            borrarMensaje.Text = $"Desea borrar el vehiculo: {vehiculo.LstrPlaca}? Registrado en {apariciones} Noticia(s).";
        ...
        private void BindGridView()
        {
            ...
            var listaTiposFuente = VehiculoBL.BuscarTodosVehiculo(ref mensajeError);
